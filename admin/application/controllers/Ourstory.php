...
class Ourstory extends CI_CONTROLLER {
	public function __construct() {
	 ...
	 public function index(){
		$data['category'] = $category =  $this->category->get_category_list(false, $page=false,'CATID','DESC',$cond=false);
		$data['tag'] = $tag =  $this->tag->get_our_story_tag(false, $page=false,'GTAGID','DESC',$cond=false);
		$this->load->view('admin/our_story/category',$data);
	}
    ...
    public function update_category(){
     ...
     public function delete_category($lid){
		...
		if($this->db->delete('ourstory_details',array('CATID'=>$lid))){
    ...
    public function change_category_status($lid){
    	$category_details = userdata('ourstory_details',array('CATID'=>$lid));
		...
		if($this->db->update('ourstory_details',$is_active,array('CATID'=>$lid))){
    	...
    	public function ourstory_content(){
		$data['subcategory'] = $subcategory =  $this->category->get_subcategory_list(false, $page=false,'SUBID','DESC',$cond=false,false);
		$data['category_details'] = $category_details = $this->category->get_category_list($limit=false,$start=false,$order_by=false,$order=false,$cond=false);

		$this->load->view('admin/our_story/sub_category',$data);
	}
...
/*print_r($tag); die();*/
		...
		$this->load->view('admin/our_story/tag',$data);
	}

	

	public function add_ourstory_tag(){
    	if (!$this->input->is_ajax_request()) {
		   exit('No direct script access allowed');
		}
    ...
    public function update_ourstory_tag(){
    	if (!$this->input->is_ajax_request()) {
		   exit('No direct script access allowed');
		}
		$this->form_validation->set_rules('TAG_NAME','Tag Name','required');
    ...
    public function delete_ourstory_tag($lid){
    	if (!$this->input->is_ajax_request()) {
		   exit('No direct script access allowed');
		}
    ...
    public function change_ourstory_tag_status($lid){
...
public function addsub(){
	 $this->ckfinder->SetupCKEditor($this->ckeditor,'../assets/ckfinder/');
		$data['subcategory'] = $subcategory =  $this->category->get_subcategory_list($limit=false,$start=false,$order_by=false,$order=false,$cond=false);
		$data['category_details'] = $category_details = $this->category->get_category_list($limit=false,$start=false,$order_by=false,$order=false,$cond=false);
		$this->load->view('admin/our_story/add_subcategory',$data);
    }
	
    public function edit($sid){
    	 $this->ckfinder->SetupCKEditor($this->ckeditor,'../../assets/ckfinder/');
    	$data['category_details'] = $category_details = $this->category->get_category_list($limit=false,$start=false,$order_by=false,$order=false,$cond=false);
    	$data['subcategory'] = $subcategory =  $this->db->get_where('ourstory_subcategory',array('SUBID'=>$sid))->row();
	...
	/*	$this->form_validation->set_rules('CATID','Category Name ','required');
		...
		if($this->form_validation->run()==FALSE){
			...
			if($this->category->add_subcategory()){
			    	redirect('ourstory/ourstory_content');
    ...
    public function delete_subcategory($lid){
		...
		if($this->db->delete('ourstory_subcategory',array('SUBID'=>$lid))){
			echo json_encode(['success' => true, 'msg' => 'Delete Successfully...']);			
		} else {
			echo json_encode(['success' => false, 'msg' => 'Technical Error...']);
		}
    ...
    public function change_subcategory_status($lid){
    	$sub_details = userdata('ourstory_subcategory',array('SUBID'=>$lid));
		...
		if($this->db->update('ourstory_subcategory',$is_active,array('SUBID'=>$lid))){
