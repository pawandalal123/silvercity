              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                if(!empty($tag)) :
                    foreach($tag as $tagkey=>$tag):
                      ?>
                        <tr id="deletetag<?php echo $tag->TAG_ID;?>">
                          ...
                          <!-- <td><?php echo $tag->NAME;?></td> -->
                              ...
                              <input type="checkbox" id="<?php echo $tag->TAG_ID;?>" <?php echo ($tag->TAG_STATUS==1) ? 'checked' : ''?> data-toggle="toggle" data-on="Active" data-off="Pending" class="tag" >
                                       ...
                                       <td> 
                                                  <span id="">
                                                      <a href="#" data-toggle="modal" data-target="#edit<?php echo $tag->TAG_ID; ?>"><i class="fa fa-edit" ></i></a>
                                                  </span>
                                                  </td>

                                                <td>
                                                  <span id="<?php echo $tag->TAG_ID;?>">
                                                    <a href="javascript:;" class="delete_tag" id="<?php echo $tag->TAG_ID;?>"><i class="fa fa-trash"></i></a>

                                                  </span>
                                            </td>   

                          <div class="modal fade" id="edit<?php echo $tag->TAG_ID;?>" tabindex="-1" role="dialog" aria-labelledby="add" aria-hidden="true">
...
<script type="text/javascript"> 
    ...
    $("form[name='add_tag']").submit(function(e) {    
        ...
        $.ajax({
            url: '<?php echo base_url('blog/add_tag')?>',
            type: "POST",
            data: formData,
            dataType: "json",
            async: false,
     ...
     $("form[name='update_tag']").submit(function(e) {    
      var formData = new FormData($(this)[0]);
      $.ajax({
        url: '<?php echo base_url('blog/update_tag')?>',
        type: "POST",
        data: formData,
        dataType: "json",
        async: false,
     ...
     //alert(tagID);    
      ...
      $('.delete_tag').click(function(){
            var categoryID  = $(this).attr('id');
             $.ajax({
              type: "GET",
              url: '<?php echo base_url('blog/delete_tag')?>'+'/'+tagID,
              dataType: "json",
              success: function(data)
              {
               $('body,html').animate({ scrollTop: 0 }, 200);
