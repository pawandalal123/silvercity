...
$this->load->view('admin/parts/header', ['title' => 'Popular ', 'add' => $add]); ?>
    <!-- Main content -->
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div class="btn-group"><span data-placement="top" data-toggle="tooltip" title="add"><a href="<?php echo base_url('category/add_category') ?>" class="btn btn-primary btnPlus dvshwalert MR5">Add Category</a></span></div>
            ...
            <div class="btn-group"><span data-placement="top" data-toggle="tooltip" title="add"><a href="<?php echo base_url('popular/add_popular') ?>" class="btn btn-primary btnPlus dvshwalert MR5"  >Add</a></span></div>
        </div>
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                 <?php 
                if(!empty($class)) :
                    foreach($class as $calkey=>$cal):
                      ?>
                        <tr id="deletecategory<?php echo $cal->CLID;?>">
                 <td><?php echo !empty($cal->IMAGE) ? '<img src="'.base_url("uploads/image/".$cal->IMAGE).'" height="50px" width="50px;">' : '';?></td> 
                          ...
                          <td><?php echo substr($cal->DESCRIPTION,0,50);?></td>
                              ...
                              <td>
                              <label>
                              <input type="checkbox" id="<?php echo $cal->CLID;?>" <?php echo ($cal->STATUS==1) ? 'checked' : ''?> data-toggle="toggle" data-on="Active" data-off="Pending" class="manish">
                              </label>
                              </td>
                                       <td> 
                                                  <span id="">
                                                       <a href="<?php echo base_url('popular/edit/'.$cal->CLID)?>"><i class="fa fa-edit" ></i></a>
                                                  </span>
                                                  </td>

                                                <td>
                                                  <span id="<?php echo $cal->CLID;?>">
                                                    <a href="javascript:;" class="delete_class" id="<?php echo $cal->CLID;?>"><i class="fa fa-trash"></i></a>
...
<script type="text/javascript"> 
   ...
   $("form[name='update_class']").submit(function(e) { 
     ...
     for (instance in CKEDITOR.instances) {
        ...
        var formData = new FormData($(this)[0]);
        $.ajax({
            url: '<?php echo base_url('popular/add_class')?>',
            type: "POST",
            data: formData,
            dataType: "json",
            async: false,
...
$('.delete_class').click(function(){
      ...
      $('.delete_teacher').click(function(){
       ...
       // alert(teachersID); die;
           ...
           $.ajax({
            type: "GET",
        url: '<?php echo base_url('popular/delete_class')?>'+'/'+teachersID,
            dataType: "json",
            success: function(data)
            {
             $.bootstrapGrowl(data.msg,{
  ...
  <script type="text/javascript">
   ...
   $(".manish").change(function() {    
    ...
    // alert(categoryID);    
      $.ajax({
           type: "GET",
           url: '<?php echo base_url('popular/change_class_status')?>'+'/'+categoryID,
           dataType: "json",
           success: function (data) {
             $('body,html').animate({ scrollTop: 0 }, 200);
                    $.bootstrapGrowl(data.msg,{
