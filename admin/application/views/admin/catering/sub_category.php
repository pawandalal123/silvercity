...
$this->load->view('admin/parts/header', ['title' => ' Content Management System', 'add' => $add]); ?>
    ...
    <section class="content">
     ...
     <!--  <div class="btn-group"><span data-placement="top" data-toggle="tooltip" title="add"><a href="<?php echo base_url('catering/addsub') ?>" class="btn btn-primary btnPlus dvshwalert MR5">Add</a></span></div> -->
        </div>
 ...
 -->                          <td><?php echo date_formats($scat->MODIFIED_ON);?></td>
                         <!--  <td><?php echo date_formats($scat->MODIFIED_ON);?></td> -->
                              ...
                              <input type="checkbox" id="<?php echo $scat->SUBID;?>" <?php echo ($scat->SUB_STATUS==1) ? 'checked' : ''?> data-toggle="toggle" data-on="Active" data-off="Pending" class="category" >
                                             ...
                                             <td>
                                                  <span id="">
                                                      <a href="<?php echo base_url('catering/edit/'.$scat->SUBID)?>" ><i class="fa fa-edit" ></i></a>
                                                  </span>
                                                  </td>
...
<script type="text/javascript"> 
  ...
  $(".category").change(function() {    
     ...
     // alert(categoryID);    
      $.ajax({
           type: "GET",
           url: '<?php echo base_url('catering/change_subcategory_status')?>'+'/'+categoryID,
           dataType: "json",
      ...
      $('.delete_subcategory').click(function(){
            var categoryID  = $(this).attr('id');
             $.ajax({
              type: "GET",
              url: '<?php echo base_url('catering/delete_subcategory')?>'+'/'+categoryID,
              dataType: "json",
              success: function(data)
              {
               $('body,html').animate({ scrollTop: 0 }, 200);
